...
DEFAULT_CHECKPOINT = str(PROJECT_ROOT / "checkpoints" / "iemocap_only" / "best_model.pth")
...
parser.add_argument('--checkpoint', type=str, default=DEFAULT_CHECKPOINT,
                    help=f'Percorso del checkpoint del modello (default: {DEFAULT_CHECKPOINT})')
...
if __name__ == "__main__":
    ...
    if not iemocap_path or not iemocap_path.exists():
        raise ValueError("❌ IEMOCAP non trovato!")
    
    print(f"✅ IEMOCAP trovato: {iemocap_path}\n")
    
    # 2. Carica dataset TEST
    print("Loading IEMOCAP test set...")
    test_dataset = CustomIEMOCAPDataset(dataset_root=str(iemocap_path), split='test')
